...
long long int sum_simd(int vals[NUM_ELEMS])
{
	...
	for (unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
		/* YOUR CODE GOES HERE */
		__m128i sum_vec = _mm_setzero_si128(), tmp, mask;
		unsigned int sum_arr[4];
		for (unsigned int i = 0; i < NUM_ELEMS / 4 * 4; i += 4) {
			tmp = _mm_loadu_si128((__m128i_u *) (vals + i));
			mask = _mm_cmpgt_epi32(tmp, _127);
			tmp = _mm_and_si128(tmp, mask);
			sum_vec = _mm_add_epi32(sum_vec, tmp);
		}
		_mm_storeu_si128((__m128i_u *) sum_arr, sum_vec);
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS])
{
	...
	for (unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
		...
		for (i = 0; i < NUM_ELEMS / 16 * 16; i += 16) {
			...
			tmp = _mm_loadu_si128((__m128i_u *) (vals + i + 4));
			mask = _mm_cmpgt_epi32(tmp, _127);
			tmp = _mm_and_si128(tmp, mask);
			sum_vec = _mm_add_epi32(sum_vec, tmp);

			tmp = _mm_loadu_si128((__m128i_u *) (vals + i + 8));
			mask = _mm_cmpgt_epi32(tmp, _127);
			tmp = _mm_and_si128(tmp, mask);
			sum_vec = _mm_add_epi32(sum_vec, tmp);

			tmp = _mm_loadu_si128((__m128i_u *) (vals + i + 12));
			mask = _mm_cmpgt_epi32(tmp, _127);
			tmp = _mm_and_si128(tmp, mask);
			sum_vec = _mm_add_epi32(sum_vec, tmp);
		}
