...
#pragma omp parallel
	{
		...
		int thread_num = omp_get_thread_num();
		for (int i = thread_num * chunk_size;
		     i < (thread_num + 1) * chunk_size && i < ARRAY_SIZE;
		     i++) {
			z[i] = x[i] + y[i];
		}
	}
...
double dotp_manual_optimized(double *x, double *y, int arr_size)
{
	...
	double global_sum = 0.0, partial_sum[omp_get_max_threads()];
...
#pragma omp for
		for (int i = 0; i < arr_size; i++)
			partial_sum[omp_get_thread_num()] += x[i] * y[i];
#pragma omp critical
		global_sum += partial_sum[omp_get_thread_num()];
	}
	return global_sum;
}
...
char *compute_dotp(int arr_size)
{
	...
	for (int i = 1; i <= num_threads; i++) {
		...
		pos += sprintf(pos,
		               "Manual Optimized: %d thread(s) took %f seconds\n",
		               i, run_time);
	...
	for (int i = 1; i <= num_threads; i++) {
		...
		run_time = omp_get_wtime() - start_time;
		pos += sprintf(pos,
		               "Reduction Optimized: %d thread(s) took %f seconds\n",
		               i, run_time);
